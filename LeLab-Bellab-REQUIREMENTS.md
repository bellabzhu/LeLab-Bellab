...
`--policy.path=...`) builds the policy entirely from scratch — for SmolVLA
this means the 350M-parameter vision-language backbone is randomly
...
training screen defaults to `lerobot/smolvla_base`. Leaving it empty (opt-in)
falls back to today's from-scratch behavior via `--policy.type`. The training
log panel surfaces which mode a run started in as its first line, so a
from-scratch run can't go unnoticed again.
